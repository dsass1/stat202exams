...
quiz(
  ...
  question_text(
    "Q2) In a typical dataframe, ___ correspond to variables. <br>  <br> 
    a) columns <br> b) observations <br> c) values <br> d) rows <br> e) cells",
    answer("wrong", correct = TRUE),
    allow_retry = TRUE,
    incorrect = "submitted"
  ),
  question_text(
  "Q3) Which of the following shows the distribution of only one numeric variable? <br>  <br> a) barplot <br> b) faceted histogram <br> c) stacked barplot <br> d) boxplot <br> e) scatterplot",
  answer("wrong", correct = TRUE),
  allow_retry = TRUE,
  incorrect = "submitted"
  ),
  ...
  question_text(
  paste("Q5) The histogram below shows the distribution of a large classes exam grades out of 100. Which of the following statements is TRUE concerning this distribution?", htmltools::img(src="images/02_grades.png", height = 300, width = 500), 
  "<br><br> a) The distribution is skewed left and the mean is less than the median. <br>
  b) The distribution is skewed right and the mean is less than the median. <br>
  c) The distribution is skewed left and the mean is greater than the median. <br>
  d) The distribution is skewed right and the mean is greater than the median. <br>
  e) The distribution is symmetric and the mean is equal to the median."),
  answer("wrong", correct = TRUE),
  allow_retry = TRUE,
  incorrect = "submitted"
  ),
  question_text(
  "Q6a) Consider the histogram from the previous problem. <br> How would the average grade change if the person with the lowest grade scored 5 percent better? <br><br> a) Increase <br> b) Decrease <br> c) Stay the same",
  answer("wrong", correct = TRUE),
  allow_retry = TRUE,
  incorrect = "submitted"
  ),
  question_text(
  "Q6b) Again, consider the histogram from the previous problem. <br> How would the IQR (Interquartile Range) of average grades change if the person with the lowest grade scored 5 percent better? <br><br> a) Increase <br> b) Decrease <br> c) Stay the same",
  answer("wrong", correct = TRUE),
  allow_retry = TRUE,
  incorrect = "submitted"
  ),
  question_text(
  "Q7a) For the statistic, MINIMUM, indicate which of the four choices is correct. <br><br> a) Measure of Centrality/Center <br> b) Measure of Variability/Spread <br> c) Both A and B <br> d) Neither A nor B",
  answer(c("wrong"), correct = TRUE),
  allow_retry = TRUE,
  incorrect = "submitted"
  ),
  question_text(
  "Q7b) For the statistic, MEDIAN, indicate which of the four choices is correct. <br><br> a) Measure of Centrality/Center <br> b) Measure of Variability/Spread <br> c) Both A and B <br> d) Neither A nor B",
  answer(c("wrong"), correct = TRUE),
  allow_retry = TRUE,
  incorrect = "submitted"
  ),
  ...
  question_text(
  "Q9) Suppose we have a dataset (some data) and we want to remove invalid data entries for one of the numerical variables and then proceed to calculate the median for that variable per level/category for a categorical variable. Which general sequence of functions/verbs would achieve this? <br><br>
  a) some data %>% filter() %>% group_by() %>% summarize()  <br>
  b) some data %>% filter() %>% count()  <br>
  c) some data %>% select() %>% group_by() %>% summarize()  <br>
  d) some data %>% select() %>% count()  <br>
  e) some data %>% arrange() %>% group_by() %>% summarize() <br>
  f) some data %>% arrange() %>% count()",
  answer("wrong",correct = TRUE),
  allow_retry = TRUE,
  incorrect = "submitted"
  )
